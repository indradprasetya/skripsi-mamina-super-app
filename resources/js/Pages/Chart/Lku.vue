...
onMounted(async () => {
    ...
    const { data } = await axios.get(route('antropometri.get', { tipe: 'lku', gender }));
    refData.value = data;
    loading.value = false;
});
...
const chartData = computed(() => {
    ...
    const recordPoints = props.child.records.map(record => ({
        ...
        y: Number(record.lingkar_kepala)
    }));
...
const chartOptions = computed(() => ({
    ...
    plugins: {
        ...
        title: {
            display: true,
            text: `Grafik LKU ${props.child.jenis_kelamin === 'L' ? 'Laki-laki' : 'Perempuan'}`,
            color: '#0f172a',
    ...
    scales: {
        ...
        y: {
            title: { display: true, text: 'Lingkar Kepala (cm)' },
...
// Penjelasan status LKU sesuai Permenkes 2/2020
function explainLKU(z) {
...
const zScore = computed(() => {
    ...
    const lingkar = Number(props.child.records[0].lingkar_kepala);
    ...
    <div v-else>
        ...
        <div class="mt-4">
            <div class="flex items-center space-x-3 mb-2">
                ...
                <div>
                    <p class="text-sm font-medium text-gray-700 dark:text-gray-300">LKU (Z-Score)</p>
